...
from src.app.backend import backgroud_job, store_data_job
...
def predict_test(backgroud_tasks: BackgroundTasks) -> Dict[str, str]:
...
def predict(data: Data, background_tasks: BackgroundTasks) -> Dict[str, str]:
    image = base64.b64encode(str(data.image_data))
    io_bytes = io.BytesIO(image)
    data.image_data = Image.open(io_bytes)
    job_id = str(uuid.uuid4())[:6]
    backgroud_job.save_data_job(
